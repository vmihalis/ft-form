...
<task type="auto">
  <name>Task 1: Add submissionCount to forms.list query and verify archive mutations</name>
  <files>convex/forms.ts</files>
  <action>
First, verify that archive/unarchive mutations exist in convex/forms.ts. The `archive` mutation exists, but `unarchive` does NOT exist. Create the `unarchive` mutation:
...
Then, modify the `list` query in convex/forms.ts to include submission count for each form.
...
```
  ...
  <verify>Run `npx convex dev` - should deploy without errors. Verify both `forms.list` includes submissionCount and `unarchive` mutation exists.</verify>
  <done>forms.list query returns submissionCount field for each form, and unarchive mutation exists for restoring archived forms</done>
</task>
