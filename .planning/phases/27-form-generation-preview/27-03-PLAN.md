...
13. **Remove the `void onComplete;` line** - it's now properly used
  ...
  <verify>
1. TypeScript compiles and lint passes:
```bash
cd /Users/memehalis/ft-form && npx tsc --noEmit && pnpm lint --quiet
```

2. Runtime verification - confirm schema extraction works with message.parts structure:
```bash
cd /Users/memehalis/ft-form && node -e "
const { extractFormSchema, mightContainSchema } = require('./src/components/ai-wizard/schema-extraction.ts');

// Simulate AI SDK v6 message.parts structure
const mockMessage = {
  role: 'assistant',
  parts: [
...
// Extract content same way as in useEffect
const content = mockMessage.parts
  .filter(p => p.type === 'text')
  .map(p => p.text)
  .join('');

console.log('Content extracted:', content.substring(0, 50) + '...');
console.log('mightContainSchema:', mightContainSchema(content));

const result = extractFormSchema(content);
console.log('Schema found:', result.found);
console.log('Schema valid:', result.valid);
console.log('Has schema object:', !!result.schema);
if (result.schema) {
  console.log('Steps count:', result.schema.steps.length);
  console.log('First step title:', result.schema.steps[0]?.title);
}
" 2>&1 || echo "Note: If module import fails, verify manually in browser devtools after pnpm dev"
```
  </verify>
  <done>
