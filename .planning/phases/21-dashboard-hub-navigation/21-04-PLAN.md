...
must_haves:
  truths:
    ...
    - "Main content uses full width on mobile (no left margin)"
    - "Main content margin adjusts dynamically based on sidebar collapse state on desktop"
  artifacts:
...
5. Closes sheet when navigation item clicked via onNavigate callback

**Dependency on SidebarNav:** This component uses the `onNavigate` prop that was added to SidebarNav in Plan 21-02.
  </action>
  <verify>
```bash
...
# Uses onNavigate callback
grep -q "onNavigate" src/components/admin/MobileNav.tsx && echo "Uses onNavigate callback"
```
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>src/components/admin/Header.tsx, src/app/admin/layout.tsx</files>
  <action>
...
Integrate Header and ensure proper responsive behavior. This plan also implements the dynamic margin based on sidebar collapse state (deferred from Plan 21-02).
...
6. No left margin on mobile (content uses full width)
7. Dynamic margin transition syncs with sidebar animation (duration-200)

**Verification note for SidebarNav change:** This plan depends on SidebarNav having the `onNavigate` prop (added in Plan 21-02). Verify SidebarNav exports this prop before MobileNav uses it:
```bash
...
# Layout has dynamic margin
grep -q "isCollapsed && \"lg:ml-\[64px\]\"" src/app/admin/layout.tsx && echo "Layout has dynamic margin"
# Verify SidebarNav has onNavigate (cross-plan dependency)
grep -q "onNavigate" src/components/admin/SidebarNav.tsx && echo "SidebarNav has onNavigate (dependency satisfied)"
# Build succeeds
...
- Main content margin adjusts dynamically when sidebar collapses (240px -> 64px)
...
# 5. Main content margin transitions smoothly when sidebar collapses

# Mobile (< 1024px):
# 1. Sidebar NOT visible
# 2. Main content uses full width (no left margin)
# 3. Hamburger menu visible in header
# 4. Tap hamburger - sheet slides in from left
# 5. Sheet contains all navigation items
# 6. Tap a nav item - sheet closes, page navigates
# 7. Theme toggle and logout still accessible in header
```
</verification>
...
6. Main content margin adjusts dynamically with sidebar collapse state on desktop
7. Mobile content uses full width (no left margin)
